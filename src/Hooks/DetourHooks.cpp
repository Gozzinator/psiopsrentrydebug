...
static void __fastcall CPlayer__Update_Hook(CPlayer* This, void* edx, int a2)
{
    ...
    if (!characterPlayer->IsDead())
    {
        ...
        if (PsiDebug::m_bGodModeCheat)
        {
            // Reuse the engine's own damage-immunity flags (same family as m_bInvisible above)
            This->m_bHealthSale = true;
            This->m_bBulletProof = true;

            // Also force-top-off health each frame as a safety net, in case
            // some damage source (fall damage, fire, scripted events) ignores those flags
            This->SetHealth(9999.0f);
        }

        if (PsiDebug::m_bInfinitePsiCheat)
        {
            This->m_bPsiSale = true;
        }
    }

    // Capture position before the engine's own movement/collision update so our
    // noclip override below isn't fought (or reverted) by that update
    CVector3 preUpdatePos{};
    if (PsiDebug::m_bNoclipCheat)
    {
        characterPlayer->GetRootAtom()->GetWorldPosition(&preUpdatePos);
    }
    ...
    if (PsiDebug::m_bNoclipCheat)
    {
        const float speed = 12.0f; // units per update - raise/lower to taste

        CVector3 facing{};
        characterPlayer->GetXZFacingVector(&facing);

        // Right vector on the XZ plane, perpendicular to facing
        CVector3 right{ -facing.z, 0.0f, facing.x };

        CVector3 move{ 0.0f, 0.0f, 0.0f };

        if (GetAsyncKeyState('W') & 0x8000)        { move.x += facing.x; move.z += facing.z; }
        if (GetAsyncKeyState('S') & 0x8000)        { move.x -= facing.x; move.z -= facing.z; }
        ...
        CVector3 newPos
        {
            preUpdatePos.x + move.x * speed,
            preUpdatePos.y + move.y * speed,
            preUpdatePos.z + move.z * speed
        };

        characterPlayer->GetRootAtom()->SetWorldPosition(&newPos);
    }
}
